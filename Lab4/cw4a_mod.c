...
int main(int argc, char *argv[])
{
    ...
    // Check whether we want to use usleep or an active approach
    int better_sleep = !!getenv("CW4_BETTER_SLEEP");
    ...
    for(i=0; i<nsmp; i++) {
        ...
        if (better_sleep) {
            unsigned long last_processing_time = time_us() - last_wakeup_time;
            if (udelsmp > last_processing_time) usleep(udelsmp - last_processing_time);
        } else {
            usleep(udelsmp);
        }
        last_wakeup_time = time_us();
