...
int main(int argc, char *argv[])
{
    ...
    // Check if we should actively wait for data
    unsigned char active_wait = 0;
    if (getenv("CW4_ACTIVE_WAIT_ALL")) active_wait = 1;
    if (ncli == 0 && getenv("CW4_ACTIVE_WAIT_0")) active_wait = 1;
    ...
    while(smpnum<nsmp) {
        ...
        if(rbuf->head != rbuf->tail[ncli]) {
            ...
            if (!active_wait) pthread_cond_wait(&rbuf->cvar,&rbuf->cvar_lock);
            pthread_mutex_unlock(&rbuf->cvar_lock);
        }
    }
    munmap(mptr,SHM_LEN);
